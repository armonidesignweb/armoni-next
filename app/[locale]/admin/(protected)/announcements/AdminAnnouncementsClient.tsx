...
export default function AdminAnnouncementsClient({ initialAnnouncements }: { initialAnnouncements: Announcement[] }) {
  ...
  const toggleActive = async (announcement: Announcement) => {
    try {
      ...
      if (res.ok) {
        setAnnouncements(prev => prev.map(a => a._id === announcement._id ? { ...a, isActive: !a.isActive } : a));
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-xl min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold font-sans text-white">Duyurular</h1>
        <button
          onClick={() => openModal()}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-neutral-700 bg-neutral-800">
              ...
              <th className="p-4 text-neutral-300 font-medium">Tarih</th>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 z-10 bg-neutral-900/90 backdrop-blur-md border-b border-neutral-800 p-6 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">
                {editingAnnouncement ? 'Duyuruyu Düzenle' : 'Yeni Duyuru Ekle'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-300 mb-1">
                    İçerik/Mesaj ({activeLang.toUpperCase()}) {activeLang === 'tr' && <span className="text-red-500">*</span>}
                  </label>
                  <textarea
                    required={activeLang === 'tr'}
                    rows={5}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-neutral-300 mb-2">Duyuru Görseli</label>
